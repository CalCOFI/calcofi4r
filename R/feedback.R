# ─── in-app feedback for the CalCOFI apps ─────────────────────────────────────
#
# The sibling of the usage log (analytics.R): one Apps Script web app, bound to a
# "CalCOFI app feedback" Sheet, that an app POSTs a feedback submission to — the
# text, the view URL, the release, viewport and theme, and an annotated PNG of
# the view (explorer plan D17). Per submission the script
#
#   1. writes the PNG to a Drive folder  CalCOFI app feedback/<app>/<ts>_<id>.png
#   2. appends a row to the Sheet's `feedback` tab (cc_feedback_header())
#   3. mails every address in the Sheet's `recipients` tab — one per row; add or
#      remove an address by editing a cell, no redeploy
#   4. files a PUBLIC issue in the app's GitHub repo (label `feedback`) with the
#      view URL, the text, the release/viewport line and the screenshot committed
#      under feedback/<id>.png so the issue can embed it — never the email, which
#      stays in the Sheet. A GITHUB_TOKEN script property enables the step
#      (fine-grained: contents + issues on that one repo); without it the step is
#      skipped and the row says so.
#
# CORS: the app POSTs `text/plain` JSON so the request stays a simple request
# (an Apps Script /exec endpoint does not answer OPTIONS). Spam: a honeypot
# field and a per-hour cap. Apps Script's 50 MB POST limit and 100–1,500
# mails/day are far above need.
#
# SETUP (one-time, by a maintainer):
#   1. Create a Sheet with two tabs: `feedback` (first row exactly
#      cc_feedback_header()) and `recipients` (A1 = "email", one address per row).
#   2. Extensions > Apps Script, paste cc_feedback_script(), Deploy > New
#      deployment > Web app, execute as "Me", access "Anyone". Copy the /exec URL.
#   3. Project settings > Script properties: GITHUB_TOKEN (optional; enables the
#      public issue). DRIVE_FOLDER_ID (optional; default: a folder made beside
#      the Sheet).
#   4. Give the app the URL: the explorer reads VITE_FEEDBACK_URL at build time;
#      a Shiny app reads CALCOFI_FEEDBACK_URL.

#' Column header for the feedback Sheet
#'
#' The exact first row of the `feedback` tab that [cc_feedback_script()] appends
#' into, so the Sheet, the script and the client payload cannot drift. The
#' submitter's `email` lives here and nowhere else — the public issue never
#' carries it.
#'
...
#' @export
#' @concept analytics
cc_feedback_header <- function()
  c("ts", "app", "url", "release", "viewport", "theme", "text", "email",
    "image_url", "issue_url", "id", "user_agent", "status")

#' Apps Script source for the feedback endpoint
#'
#' The `doPost()` handler to paste into the feedback Sheet's bound Apps Script
#' project (see the setup notes at the top of `R/feedback.R`). The client posts
#' one JSON object as `text/plain`:
#' `{app, url, release, viewport, theme, text, email, image (data:image/png;base64,…), website}`
#' — `website` is the honeypot and must be empty.
#'
#' @param repos named character vector: which GitHub repository each `app`
#'   files its public issue in (`c(explore = "CalCOFI/explore")`); an app not
#'   listed here files no issue
#' @param label the issue label
#' @param max_per_hour the spam cap: submissions accepted per hour, across all
#'   apps
#' @param branch the branch the screenshot is committed to (the raw URL the
#'   issue embeds points at it)
#' @return character scalar of JavaScript source
#' @examples
#' cat(cc_feedback_script())
#' @export
#' @concept analytics
cc_feedback_script <- function(repos = c(explore = "CalCOFI/explore"), label = "feedback",
                               max_per_hour = 20L, branch = "main") {
  stopifnot(is.character(repos), !is.null(names(repos)), all(nzchar(names(repos))),
            all(grepl("^[^/]+/[^/]+$", repos)), is.numeric(max_per_hour), max_per_hour >= 1)
  cols <- cc_feedback_header()
  glue::glue(
...
var REPOS = <<jsonlite::toJSON(as.list(repos), auto_unbox = TRUE)>>;
var LABEL = <<jsonlite::toJSON(label, auto_unbox = TRUE)>>;
var BRANCH = <<jsonlite::toJSON(branch, auto_unbox = TRUE)>>;
var MAX_PER_HOUR = <<as.integer(max_per_hour)>>;
