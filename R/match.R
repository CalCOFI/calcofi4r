...
#' `"v2026.08.07"`. Reads the same `latest.txt` every other CalCOFI consumer
#' reads, so it never disagrees with them.
#'
#' Use it when you are about to pin. An analysis that passes `version =
#' "latest"` is not reproducible — it silently follows the promoted release — but
#' you cannot pin to a version you have not looked up. Record the value beside
#' your results, along with the package version that read it: immutable data is
#' only half of reproducibility, since a later `calcofi4r` may expect a schema an
#' older release does not carry.
#'
#' @return Character scalar, the current release version.
#' @export
#' @concept database
#' @examples
#' \dontrun{
...
.cc_read_parquet <- function(version) {
  ...
  physical <- function(table) cc_read_parquet_sql(cc_release_sources(cat_, table), prefer_single_file = TRUE)
  # a catalog view (obs over obs_bio + obs_env since v2026.09) expands to its SQL
  # over the tables it reads, parenthesised so it stands wherever a
  # read_parquet(...) stood — 1:1 with db-query lib/release.js readParquetFor()
  function(table) {
    if (table %in% names(views)) paste0("(", cc_view_sql(cat_, table, physical), ")") else physical(table)
  }
}
