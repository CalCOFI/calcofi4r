...
a query touches, straight over HTTPS. `cc_get_db()` registers every
release table as a view, so you query by bare table name:
...
for the column contract and `cc_match_bio_env()` for your own
subqueries.
...
`cc_get_db()` DuckDB connection as the catalog `pg`, so one query can
join the public release to the team’s working state. See
`?cc_pg_connect`.
...
`cc_release_sources()` → `cc_read_parquet_sql()` is how every table here
is resolved, and you can hand that SQL fragment to any DuckDB. Details:
