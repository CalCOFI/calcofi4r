...
The `doPost()` handler to paste into the feedback Sheet's bound Apps
Script project (see the setup notes at the top of `R/feedback.R`). The
client posts one JSON object as `text/plain`:
...
- repos:
  ...
  named character vector: which GitHub repository each `app` files its
  public issue in (`c(explore = "CalCOFI/explore")`); an app not listed
  here files no issue
...
- branch:
  ...
  the branch the screenshot is committed to (the raw URL the issue
  embeds points at it)
