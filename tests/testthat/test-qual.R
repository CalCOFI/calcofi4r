...
test_that("cc_climatology() leaves flagged CTD values out of the mean", {
  ...
  DBI::dbWriteTable(con, "sample", data.frame(
    sample_key = paste0("S", 1:4), site_key = "090.0 050.0", stringsAsFactors = FALSE))
  ...
  expect_equal(cl$site_key, "090.0 050.0")
  expect_equal(cl$grid_key, "st50-ln90")
})
