...
### All Zones, including Historical Station Pattern

```{r map_contours}
# show first 6 rows of example data frame with temperature in space as variable
head(cc_bottle)

# calculate map contours, aka isobands, of variable using generalized additive model (GAM)
v_ply_all <- map_contours(
  df = cc_bottle %>%
    ...
    summarize(
      v = mean(t_degc),
      .groups = "drop"), 
  ply = cc_grid_zones) # for all zones, including historical
...
mapView(v_ply_extended, zcol="v", layer.name="temp(ºC)")
```
...
v_ply_inshore <- map_contours(
  ...
  ply = cc_grid_zones %>% 
    filter(
      ...
      sta_shore    == "nearshore"))
mapView(v_ply_inshore, zcol="v", layer.name="temp(ºC)")
```
