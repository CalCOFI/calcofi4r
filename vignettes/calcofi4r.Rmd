...
mapView(st_as_stars(r_k_ext), layer.name="Temp.(ºC)")
```

```{r}
# generate contour polygons from raster
p_k_ext <- rast_to_contours(r_k_ext, aoi_ext)
mapView(p_k_ext, zcol = "z_avg", layer.name="Temp.(ºC)")
```
...
mapView(st_as_stars(r_k_stdnr), layer.name="Temp.(ºC)")
```

```{r}
# generate contour polygons from raster
p_k_stdnr <- rast_to_contours(r_k_stdnr, aoi_stdnr)
mapView(p_k_stdnr, zcol = "z_avg", layer.name="Temp.(ºC)")
```
...
# show map contours
mapView(p_g_all, zcol="v", layer.name="Temp.(ºC)")
```
...
mapView(p_g_ext, zcol="v", layer.name="Temp.(ºC)")
```
...
# display map contours
mapView(p_g_stdnr, zcol="v", layer.name="Temp.(ºC)")
```
-->
